...
interface TrackingTokenResponse {
  token: string;
...
function StatusIllustration({ status }: { status: OrderStatus }) {
  ...
  return <div className="text-8xl text-center mb-6">{emoji}</div>;
}

// ── Tracking token fetcher ────────────────────────────────────────────────────

/**
 * Fetches a short-lived HMAC tracking token from the server.
 * The server validates order ownership before issuing the token, so this will
 * reject (throw) if the caller does not own the order or the order is terminal.
 */
async function fetchTrackingToken(orderId: string): Promise<string> {
  const response = await api.get<TrackingTokenResponse>(
    `/customer/orders/${orderId}/tracking-token`,
  );
...
export default function OrderTrackPage() {
  ...
  const { data: order } = useQuery({
    queryKey: ["order", id],
    queryFn: () =>
      api.get<OrderDetail>(`/customer/orders/${id}`).then((r) => r.data),
    enabled: !!id,
  ...
  // SignalR — fetch tracking token, then join order group + listen for updates
  useEffect(() => {
    if (!id) return;
    ...
    hub.on(
      ...
      (payload: { orderId: string; newStatus: string }) => {
        if (payload.orderId === id) setStatus(payload.newStatus as OrderStatus);
      },
    );
    ...
    // On reconnect the connection gets a new connectionId, so the server-side
    // group membership is lost. Re-fetch a fresh token and rejoin the group.
    hub.onreconnected(async () => {
      try {
        const token = await fetchTrackingToken(id);
        await hub.invoke("TrackOrder", id, token);
      } catch (err) {
        console.error("Failed to rejoin tracking group after reconnect:", err);
      }
    });
    ...
    hub
      ...
      .then(async () => {
        try {
          // Fetch a signed token proving this user owns the order, then pass
          // it to TrackOrder so the hub can verify ownership without a DB hit.
          const token = await fetchTrackingToken(id);
          await hub.invoke("TrackOrder", id, token);
        } catch (err) {
    ...
    <main className="min-h-screen bg-gray-50">
      <div className="max-w-md mx-auto px-4 py-6 min-h-screen flex flex-col">
        ...
        <div className="flex items-center justify-between mb-4">
          <Link
            to="/menu?lat=30.0444&lng=31.2357"
          ...
          {order?.items && (
            <div className="flex gap-2 mt-3 flex-wrap">
              {order.items.slice(0, 4).map((item, i) => (
                <div
                  key={i}
                  className="w-10 h-10 rounded-full bg-gray-100 overflow-hidden border-2 border-white shadow-sm"
                >
                  <img
                    src={
                      item.mealImageUrl
