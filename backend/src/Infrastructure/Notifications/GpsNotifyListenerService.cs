    ...
    ILogger<GpsNotifyListenerService> logger) : BackgroundService
{
    ...
    private void OnGpsNotification(object sender, NpgsqlNotificationEventArgs e)
    {
        try
        {
            ...
            // Errors are caught and logged so a bad tick never silently kills the pipeline.
            _ = Task.Run(async () =>
            {
                try
                {
                ...
                {
                    // Log but don't rethrow — a single failed push is non-fatal.
                    // The next GPS tick will arrive within seconds.
                    logger.LogError(ex,
                        "Failed to push GPS update to SignalR for order {OrderId} driver {DriverId}",
