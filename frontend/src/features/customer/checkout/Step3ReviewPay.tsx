...
import OrderLocationConfirmModal from "@/components/ui/OrderLocationConfirmModal";
...
export default function Step3ReviewPay() {
  ...
  // ── Geo location ───────────────────────────────────────────────────────────
  const { location, isExpired } = useGeoStore();
  ...
  const placeOrderMutation = useMutation({
    mutationFn: async () => {
      if (!customerInfo || !userId || !chefId)
        throw new Error("Missing order data");

      if (!location) throw new Error("Missing delivery location");

      return await ordersApi.place({
        chefId,
  ...
  // ── Place order — gate behind location freshness check ────────────────────
  ...
  return (
    <>
            ...
            {customerInfo && (
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  <p className="font-medium">
                    {customerInfo.district}, {customerInfo.street}
                  </p>
                  <p className="text-[--text-muted]">
                    Floor No. {customerInfo.floorNo}, Apartment No.{" "}
                    {customerInfo.apartmentNo}.
                  </p>
                </div>
              </div>
            )}
      ...
      {/* Re-confirm location if geo is stale at order time */}
