...
public sealed class OrderItem : Entity
{
    ...
    /// </summary>
    public Money VariantPrice { get; private set; } = null!;

    public int Quantity { get; private set; }

private string _sideDishesJson
{
    get => JsonConvert.SerializeObject(_sideDishes);
    set => _sideDishes.AddRange(
        JsonConvert.DeserializeObject<List<SelectedSideDishSnapshot>>(value) ?? []);
}

private string _toppingsJson
{
    get => JsonConvert.SerializeObject(_toppings);
    set => _toppings.AddRange(
        JsonConvert.DeserializeObject<List<SelectedToppingSnapshot>>(value) ?? []);
}
