    ...
    : IDomainEventHandler<OrderReadyForPickupDomainEvent>
{
    ...
    public async Task Handle(
        ...
        CancellationToken cancellationToken)
    {
        // Step 1: Notify customer their order is ready
        await notificationService.NotifyCustomerOrderStatusAsync(
            domainEvent.CustomerId,
            domainEvent.OrderId,
        ...
        // Step 4: Build the fat payload once — reused for all drivers in the fan-out.
        // Loading the order + chef here avoids a redundant DB round-trip per driver
        // and ensures every driver receives identical, consistent data.
        ...
        if (order is null || chef?.ChefProfile is null)
        {
            logger.LogError(
                "Could not build delivery payload for order {OrderId} — order or chef missing",
                domainEvent.OrderId);
            return;
        }
        ...
        var payload = new AvailableDeliveryResponse(
            OrderId: order.Id,
            ChefId: chef.Id,
            ChefName: chef.FullName,
            PickupLatitude: chef.ChefProfile.OperationLocation.Latitude,
            PickupLongitude: chef.ChefProfile.OperationLocation.Longitude,
            PickupAddress: chef.ChefProfile.OperationLocation.AddressLine,
            DropoffLatitude: order.DeliveryLocation.Latitude,
            DropoffLongitude: order.DeliveryLocation.Longitude,
            DropoffAddress: order.DeliveryLocation.AddressLine,
            Total: total.Amount,
            Currency: total.Currency,
            Items: order.Items.Select(i => new AvailableDeliveryItemResponse(
                MealName: i.MealName,
        ...
        // Step 5: Fan-out — one push per driver, all concurrent
