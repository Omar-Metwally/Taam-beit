...
```bash
# 1. Clone
git clone https://github.com/Omar_Metwally/taambeit.git
cd taambeit
...
| **Garage S3 API** | http://localhost:3900 (authenticated uploads) |
| **Garage Public Web** | http://localhost:3903 (public files) |
...
docker compose up db garage -d
# Note: On first run, you must also run:
#   docker compose up garage-bootstrap && sh update-env.sh
...
- **Garage** — Lightweight, S3-compatible object storage (meal images, private documents)
